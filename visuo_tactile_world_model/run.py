...
from visuo_tactile_world_model.world_model.runner.base_runner_pipeline import BaseRunnerPipeline
from visuo_tactile_world_model.world_model.utils.config import ConfigNode
...
def _is_training_ckpt(ckpt_path: str) -> bool:
    """Return True if the safetensors file is a training checkpoint.

    Training checkpoints saved with remove_prefix_in_ckpt: pipe. have keys
    prefixed with 'dit.', 'tactile_tokenizer.', 'tactile_head.', etc.
    Base model shards (DiT / T5 / VAE) use unprefixed keys.
    """
    try:
        ...
        with safe_open(ckpt_path, framework="pt", device="cpu") as f:
            keys = list(f.keys())
        return any(
            k.startswith(("dit.", "tactile_tokenizer.", "tactile_head."))
            for k in keys[:30]
        )
    except Exception:
        return False
...
def apply_ckpt_override(cfg: dict, ckpt_path: str) -> dict:
    ...
    if _is_training_ckpt(ckpt_path):
        print(f"[ckpt] Detected training checkpoint — routing to runner.params.train_ckpt")
        runner_params = cfg.setdefault("runner", {}).setdefault("params", {})
        runner_params["train_ckpt"] = ckpt_path
        return cfg
...
def apply_output_convention(cfg: dict, config_name: str, run_id: str):
    ...
    keep_config_output = os.environ.get("LIGHTEWM_KEEP_CONFIG_OUTPUT_DIR", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
    ...
    if task == "train":
        if keep_config_output and runner_params.get("output_path"):
            return Path(str(runner_params["output_path"]))
        runner_params["output_path"] = str(run_dir)
    elif task == "infer":
        if keep_config_output and runner_params.get("output_dir"):
            return Path(str(runner_params["output_dir"]))
        runner_params["output_dir"] = str(run_dir / "infer_output")
    return run_dir
...
def main():
    parser = argparse.ArgumentParser(description="Visuo-tactile world model config runner")
