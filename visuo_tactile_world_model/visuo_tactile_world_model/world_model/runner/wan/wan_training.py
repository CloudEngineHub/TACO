...
from visuo_tactile_world_model.world_model.model.loss import DirectDistillLoss, FlowMatchJointDenoiseLoss, FlowMatchSFTLoss
from visuo_tactile_world_model.world_model.runner.loops import launch_training_task
from visuo_tactile_world_model.world_model.runner.base_pipeline import PipelineUnit
from visuo_tactile_world_model.world_model.runner.runner_util.instantiation import instantiate_component_from_section
from visuo_tactile_world_model.world_model.runner.runner_util.wan_runtime import (
    build_wan_i2v_pipeline_from_params,
    build_wan_i2v_runtime_args,
    build_wan_training_dataset,
)
from visuo_tactile_world_model.world_model.utils.loader import load_state_dict
from visuo_tactile_world_model.world_model.utils.logger import ModelLogger
...
class WanTrainingModule(torch.nn.Module):
    def __init__(
        ...
        self.task_to_loss = {
            ...
            "sft_joint_denoise:data_process": lambda pipe, *args: args,
            "sft": lambda pipe, inputs_shared, inputs_posi, inputs_nega: FlowMatchSFTLoss(pipe, **inputs_shared, **inputs_posi),
            "sft:train": lambda pipe, inputs_shared, inputs_posi, inputs_nega: FlowMatchSFTLoss(pipe, **inputs_shared, **inputs_posi),
            "direct_distill": lambda pipe, inputs_shared, inputs_posi, inputs_nega: DirectDistillLoss(pipe, **inputs_shared, **inputs_posi),
            "direct_distill:train": lambda pipe, inputs_shared, inputs_posi, inputs_nega: DirectDistillLoss(pipe, **inputs_shared, **inputs_posi),
            "sft_joint_denoise": lambda pipe, inputs_shared, inputs_posi, inputs_nega: FlowMatchJointDenoiseLoss(pipe, **inputs_shared, **inputs_posi),
            "sft_joint_denoise:train": lambda pipe, inputs_shared, inputs_posi, inputs_nega: FlowMatchJointDenoiseLoss(pipe, **inputs_shared, **inputs_posi),
        }
        self.tactile_loss_weight = tactile_loss_weight
        self.tactile_sequence_key = tactile_sequence_key
        # Per-channel z-score stats for tactile/force; applied only at :train time
        # so cache writes (sft_joint_denoise:data_process) keep raw values.
        self.tactile_mean = None
        self.tactile_std = None
        if tactile_stats_path:
            stats = np.load(tactile_stats_path)
            self.tactile_mean = torch.from_numpy(stats["mean"]).float().view(1, 1, -1)
            self.tactile_std = torch.from_numpy(stats["std"]).float().clamp_min(1e-6).view(1, 1, -1)
        self.max_timestep_boundary = max_timestep_boundary
        self.min_timestep_boundary = min_timestep_boundary
        self._last_loss_components = {}

    def _normalize_tactile(self, tactile: torch.Tensor) -> torch.Tensor:
        if self.tactile_mean is None or self.tactile_std is None:
            return tactile
        mean = self.tactile_mean.to(device=tactile.device, dtype=tactile.dtype)
        std = self.tactile_std.to(device=tactile.device, dtype=tactile.dtype)
        return (tactile - mean) / std


    def _prepare_cached_inputs(self, inputs):
        inputs_shared = dict(inputs[0])
        configured = set(self.extra_inputs)
        optional_keys = {
            "tactile_sequence": "tactile_input",
            "force_sequence": "tactile_input",
        }
        for config_key, shared_key in optional_keys.items():
            if config_key not in configured:
                inputs_shared.pop(shared_key, None)
        ...
        inputs_shared.setdefault("tactile_loss_weight", self.tactile_loss_weight)
        return inputs_shared, inputs[1], inputs[2]
    ...
    def parse_extra_inputs(self, data, extra_inputs, inputs_shared):
        for extra_input in extra_inputs:
            ...
            elif extra_input in ("tactile_sequence", "force_sequence"):
                tactile = data.get(extra_input)
                if tactile is None:
                    raise ValueError(
                        f"extra_input '{extra_input}' is configured but the sample does not provide it. "
                        f"Sample keys: {list(data.keys())}"
                    )
                if tactile.ndim == 2:
                    tactile = tactile.unsqueeze(0)
                # Joint-denoise target: noised and predicted by WanTactileHead.
                inputs_shared["tactile_input"] = tactile
            else:
                raise ValueError(
                    f"Unsupported extra input '{extra_input}' in current Wan training pipeline. "
                    "Supported: 'input_image', 'tactile_sequence', 'force_sequence'."
                )
    ...
    def get_pipeline_inputs(self, data):
        ...
        inputs_shared = {
            ...
            "tactile_loss_weight": self.tactile_loss_weight,
        }
    ...
    def forward(self, data, inputs=None):
        if inputs is None:
            ...
            inputs = self._prepare_cached_inputs(inputs)
        # Apply normalization only for training (data_process must cache raw
        # values so different stats can be tried later).
        if self.task.endswith(":train") and self.tactile_mean is not None:
            inputs_shared = inputs[0]
            tactile = inputs_shared.get("tactile_input")
            if isinstance(tactile, torch.Tensor):
                inputs_shared["tactile_input"] = self._normalize_tactile(tactile)
                inputs = (inputs_shared, inputs[1], inputs[2])
        inputs = self.transfer_data_to_device(inputs, self.pipe.device, self.pipe.torch_dtype)
        for unit in self.pipe.units:
            inputs = self.pipe.unit_runner(unit, self.pipe, *inputs)
        loss = self.task_to_loss[self.task](self.pipe, *inputs)
        self._last_loss_components = getattr(self.pipe, "_last_loss_components", {})
        return loss
...
class WanTrainRunner:
    ...
    def run(self):
        ...
        pipe = build_wan_i2v_pipeline_from_params(
            {
                ...
                "tactile_dim": getattr(args, "tactile_dim", None),
                "tactile_hidden_dim": getattr(args, "tactile_hidden_dim", None),
                "tactile_num_layers": getattr(args, "tactile_num_layers", None),
                "train_ckpt_path": getattr(args, "train_ckpt_path", None),
            },
        ...
        model = WanTrainingModule(
            ...
            tactile_loss_weight=getattr(args, "tactile_loss_weight", 0.1),
            tactile_sequence_key=getattr(args, "tactile_sequence_key", "force_sequence"),
            tactile_stats_path=getattr(args, "tactile_stats_path", None),
        )
        ...
        if validation_dataset_section is not None:
            ...
            validator = PeriodicWanVideoValidator(
                ...
                num_samples=int(getattr(args, "validation_num_samples", 1)),
