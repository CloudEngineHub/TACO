...
from visuo_tactile_world_model.world_model.runner.loops import launch_data_process_task
    ...
    hash_key = json.dumps(
        {
            ...
            "fps": args.fps,
            # Bumped when the expanded record schema changes (e.g. side-stream
            # wrapping into context-window dicts). Increment to invalidate
            # previously cached jsons.
            "schema_version": 2,
        },
        sort_keys=True,
    )
    ...
    for record_id, record in enumerate(records):
        ...
        if total_frames < window_size:
            ...
            if args.context_window_tail_align:
                # Cover every frame position not already reached by the stride
                # windows. Positions k in [last_covered+1, N-1] produce windows
                # [k, k+W] that exceed the video and need last-frame padding,
                # ensuring every original frame appears as a training start frame.
                last_covered = starts[-1] if starts else -1
                for pos in range(last_covered + 1, total_frames):
                    starts.append(pos)
                    tail_starts.add(pos)

        for context_start in starts:
            # Tail windows (including short-video pad) always use pad_last=True
            # so that fps-processing discrepancies don't silently shift the clip
            # leftward — instead the last frame is repeated to fill window_size.
            pad_last = context_start in tail_starts
            ...
            for side_key in ("force_sequence", "tactile_sequence"):
                ...
                if isinstance(side_val, str):
                    expanded[side_key] = {"path": side_val, **window_meta}
