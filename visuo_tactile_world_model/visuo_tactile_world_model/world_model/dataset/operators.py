...
class LoadNumpyArray(DataProcessingOperator):
    """Load a `.npy` file and return a float32 torch.Tensor.
    ...
    Slicing semantics mirror `LoadVideo.build_sequence_ids`:
      - If end exceeds available frames and `pad_last=True`, the last frame is
        repeated.
      - If end exceeds available frames and `pad_last=False`, the window is
        shifted backward (tail-align fallback).
    After slicing, if `num_frames` is set and differs from the slice length,
    the slice is uniformly resampled with `np.linspace + round`.
    """
    ...
    def __call__(self, data):
        ...
        if self.frame_ids is not None:
            ...
            total = arr.shape[0]
            idx = np.linspace(0, max(total - 1, 0), num=self.num_frames).round().astype(np.int64)
